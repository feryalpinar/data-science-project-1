...
def question_8_query():
    connection = connect_db()
    cursor = connection.cursor()
    cursor.execute('SELECT customer_id, email FROM customers WHERE email IS NOT NULL ORDER BY customer_id LIMIT 5')
    data = cursor.fetchall()
    cursor.close()
    connection.close()
    return data
...
def question_9_query():
    connection = connect_db()
    cursor = connection.cursor()
    cursor.execute('SELECT order_id,total_amount FROM orders ORDER BY total_amount LIMIT 3')
    data = cursor.fetchall()
    cursor.close()
    connection.close()
    return data
